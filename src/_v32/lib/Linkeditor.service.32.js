import { viewMode, currentFile } from "../../lib/store.js";
import { FileService32 } from "./File.service.32.js";
import { Parser } from "../../lib/Parser";
import { Permission, registerFileAction, addNewFileMenuEntry, DefaultType } from "@nextcloud/files-legacy";
...
export class LinkeditorService32 {
	/**
	...
	static registerFileActions() {
		...
		registerFileAction({
			...
			exec: async (file) => {
				await LinkeditorService32.loadAndChangeViewMode({
							...
							onCreate: async (file) => {
								...
								currentFile.update(() =>
									FileService32.getFileConfig({
										...
										isNew: true,
										onCreate: async (file) => {
											await LinkeditorService32.saveAndChangeViewMode(file);
										},
									}),
								);
							},
	...
	static async loadAndChangeViewMode({ fileName, dirName, nextViewMode, publicUser, downloadUrl, permissions }) {
		...
		let file = {};
		if (publicUser) {
			file = await FileService32.loadPublic({ downloadUrl });
		} else {
			file = await FileService32.load({ fileName, dir: dirName });
		}
		if (file) {
	...
	static async saveAndChangeViewMode({ name, dir, url, fileModifiedTime, sameWindow, skipConfirmation }) {
		...
		// Save file
		await FileService32.save({ fileContent, name, dir, fileModifiedTime });
		// Hide editor
