// Adapted from https://gist.github.com/manekinekko/e897e5025048cfa10fedcfd6317aab5d
// Ported to browser from https://github.com/joeferner/node-bplist-parser/blob/master/bplistParser.js
...
export class BinaryPlistParserService {
	...
	parse64Content(base64Content) {
		...
		const array = new BrowserBuffer(rawLength);
	...
	parseBuffer(buffer) {
		...
		const header = buffer.toString("utf8", 0, l);
		if (header !== "bplist") {
		...
		const parseObject = (tableOffset) => {
			...
			const parsePlistString = (isUtf16) => {
				...
				length *= isUtf16 + 1;
				if (length < maxObjectSize) {
					let plistString = buffer.toString("utf8", offset + stroffset, offset + stroffset + length);
					if (isUtf16) {
